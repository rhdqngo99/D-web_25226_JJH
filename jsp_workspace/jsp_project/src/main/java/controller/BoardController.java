...
//				파일 첨부가 있을 경우 사용
				...
				// reques의 객체를 다루기 쉽게 변환해주는 클래스
				ServletFileUpload fileUpload = new ServletFileUpload(fileItemFactory);
				
				List<FileItem> itemList = fileUpload.parseRequest(request);
				
				for(FileItem item : itemList) {
					// item => title, writer, content => text imageFile => imge
					...
					case "imageFile":
						...
						if(item.getSize()>0) {
							...
							try {
								...
								//썸네일 작업 : 리스트 페이지에서 트레픽 과다 사용 방지 (연결시간 지연 방치)
...
//				log.info(">> insert {}", (isOk > 0)? "성공" : "실패");
		...
		case "list": 
				try {
					...
					// select * from board orderby bno limit pageStart, qty;
		...
		case "remove":
			try {
				...
				if(bvo.getImageFile() != null) {
					...
					fh.deleteFile(savePath, bvo.getImageFile());
				}
